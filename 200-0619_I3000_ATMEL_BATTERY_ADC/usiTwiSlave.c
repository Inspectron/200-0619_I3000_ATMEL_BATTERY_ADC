...
#define PIN_USI             PINA
#define PORT_USI_SDA        PORTA6   //PB5
#define PORT_USI_SCL        PORTA4   //PB7
...
// TWI Slave defines
#define TWI_SLA 0xCE  // Slave Address
uint8_t	gTWISlaveRead;
...
ISR( USI_OVERFLOW_VECTOR )
{
  ...
  switch ( overflowState )
  {
    ...
    case USI_SLAVE_CHECK_ADDRESS:
	  //tmpReg = USIDR;
      if ( ( USIDR == 0 ) || ( ( USIDR >> 1 ) == TWI_SLA>>1) )
      {
        if ( USIDR & 0x01 )
        {
    	  tmpReg = rxBuf[rxHead];
		  usiTwiTransmitByte(gTWIRegisters[tmpReg]);
    ...
    case USI_SLAVE_CHECK_REPLY_FROM_SEND_DATA:
      if ( USIDR )
      {
        ...
        SET_USI_TO_TWI_START_CONDITION_MODE( );
		gnBtyesinBuf = 0;							// rjr!!! clear counter on reads; otherwise the next read increments and makes it look like we have a write request in main! but this makes the writes afu...
		rxHead = rxTail = 0;						// rjr!!! try resetting this here!
        return;
      }
    ...
    case USI_SLAVE_GET_DATA_AND_SEND_ACK:
	  ...
	  gnBtyesinBuf++;
	  if(gnBtyesinBuf >= 2) gTWISlaveWriteRequest = true;
